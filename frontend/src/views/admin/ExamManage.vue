    ...
    <div class="management-card">
      <div class="card-header">
        ...
        <div class="card-actions">
          <button class="action-btn refresh-btn" @click="loadExams">
          ...
          <el-table-column prop="examName" label="考试名称" min-width="180">
            <template #default="{ row }">
              <div class="exam-name-cell">
                ...
                <div v-if="row.courseName" class="course">📚 {{ row.courseName }}</div>
              </div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="320" align="right">
            <template #default="{ row }">
              <div class="action-buttons">
                ...
                <button class="action-btn delete-btn" @click="delExam(row)" title="删除">
                  <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2">
...
const loadExams = async () => {
  const res = await request.get("/api/admin/exam/page", { params: { current: 1, size: 50, ...query } });
  exams.value = res.data?.records || [];
};

const openAdd = () => {
  editId.value = null;
  Object.assign(form, { examName: "", location: "", time: "", classNo: "", majorId: undefined });
...
const submit = async () => {
  if (!form.examName?.trim()) return ElMessage.warning("请输入考试名");
  if (!form.majorId) return ElMessage.warning("请选择专业");
  if (editId.value) {
    await request.put(`/api/admin/exam/${editId.value}`, form);
  } else {
    await request.post("/api/admin/exam/add", form);
  }
...
const toggleStatus = async (row) => {
  await request.patch(`/api/admin/exam/${row.id}`);
  ElMessage.success("操作成功");
  loadExams();
};

const delExam = async (row) => {
  await ElMessageBox.confirm("确定删除该考试吗？", "提示", { type: "warning" });
  await request.delete("/api/admin/exam", { data: [row.id] });
